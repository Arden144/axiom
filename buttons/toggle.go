...
var Toggle = bot.Button{
	Query: "toggle",
	Handler: func(ctx context.Context, e bot.ButtonEvent, msg *discord.MessageCreate) error {
		player := bot.GetPlayer(*e.GuildID())

		if !player.Playing() {
			*msg = msg.WithContent("There's nothing playing.")
			return nil
		}
		...
		if player.Paused() {
			...
			*msg = msg.WithEmbeds(embeds.Play(player.Track().Info))
		} else {
